...
class inductive_cmd_fn {
    ...
    */
    ...
    void parse_intro_rules(bool has_params, ast_data & parent, expr const & ind, buffer<expr> & intro_rules,
                           buffer<optional<std::string>> & intro_rule_docs, bool prepend_ns) {
        auto& asts = m_p.new_ast("intros", m_p.pos());
        parent.push(asts.m_id);
        ...
        while (true) {
            ...
            asts.push(doc_id).push(ir_ast).push(kind_id).push(bis.m_id);
    ...
    expr parse_inductive(buffer<expr> & params, buffer<expr> & intro_rules, buffer<optional<std::string>> & intro_rule_docs) {
        ...
        expr ind = parse_single_header(m_p, data, nscope, m_lp_names, params);
        ...
        m_p.add_local(ind);
        data.push(m_p.parse_local_notation_decl());

        parse_intro_rules(!params.empty(), data, ind, intro_rules, intro_rule_docs, false);
    ...
    void parse_mutual_inductive(buffer<expr> & params, buffer<expr> & inds, buffer<buffer<expr> > & intro_rules,
        ...
        m_p.cmd_ast_data().push(m_p.parse_local_notation_decl()).push(ind_asts.m_id);
        ...
        for (expr const & pre_ind : pre_inds) {
            ...
            parse_intro_rules(!params.empty(), ind_ast, ind, intro_rules.back(), intro_rule_docs.back(), false);
            inds.push_back(ind);
        }
...
public:
    ...
    void parse(parse_result & result) {
        ...
        m_p.cmd_ast_data().push(m_meta_info.m_modifiers_id);
