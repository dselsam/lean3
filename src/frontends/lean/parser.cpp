...
expr parser::parse_led(expr left) {
    if (is_sort_wo_universe(left) &&
        ...
        ast_id id; level l;
        std::tie(id, l) = parse_level(get_max_prec());
        if (data.m_children.empty()) {
            lean_assert(!"sort_wo_universe is missing metadata");
            maybe_throw_error({"sort_wo_universe is missing metadata", pos()});
        } else {
            data.m_children[0] = id;
        }
