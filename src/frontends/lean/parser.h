...
public:
    ...
    ast_id cmd_ast() const { lean_assert(!m_commands.empty()); return m_commands.back(); }
    ast_data & cmd_ast_data() { return *m_ast[cmd_ast()]; }
    pos_info cmd_pos() const override { return m_commands.empty() ? pos_info() : ast_pos(cmd_ast()); }
